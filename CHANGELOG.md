...
- コミット詳細とリビジョン比較を別ウィンドウで開けるようにしました
- バイナリファイルの16進表示、SSHキーの一覧表示と生成を追加しました
- コミットグラフのハイライト切り替え、比較対象の片側だけにあるコミット一覧、統計のブランチ絞り込みを追加しました
- gitflow-next、追加の外部ツール・ターミナル、Erlang・OCaml・Swiftの構文強調に対応しました
- AIコミットメッセージ生成に現在ブランチの情報と、OpenAI系の推論量設定を追加しました

### 変更

- 履歴フィルターの折りたたみ、ワークツリーのブランチ検索、コミット本文の桁ガイド、キーボード・マウス操作を改善しました
- リモートごとの自動フェッチ除外、タグを取得しないリモート追加、新しいリモートブランチへのプッシュに対応しました

### 修正

- 非UTF-8ファイルや改行末尾を含む差分の部分ステージ・逆適用で、元の内容が変わることがあった問題を修正しました
- 通信操作のキャンセルをGitプロセスへ伝え、画面切り替えやウィンドウ終了後に古い読み込み結果が反映される問題を修正しました
- 引用符や末尾の区切り文字を含むパス・参照名を扱うプル、ワークツリー、Git LFSなどの引数処理を修正しました
- コンフリクトの照会失敗を解決済みと扱う問題や、バイナリ競合・サブモジュール・画像差分の判定を修正しました
- 自動スタッシュを伴う操作で、既存スタッシュや未コミット変更を誤って扱うことがあった問題を修正しました
