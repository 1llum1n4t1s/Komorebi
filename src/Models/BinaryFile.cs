...
public class BinaryFile : IDisposable
{
    ...
    public BinaryFile(string file, bool needDeleteFile = false)
    {
        ...
        if (File.Exists(_filePath))
        {
            ...
            _reader = new FileStream(_filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete, BUFFER_SIZE, FileOptions.RandomAccess);
            _readedStart = 0;
    ...
    {
        if (_reader == null || _size == 0 || offset < 0 || length <= 0 || offset >= _size)
        ...
        {
            // 作業ツリーの同時書き換え・切り詰めで描画処理を落とさない。
            _readedEnd = _readedStart;
            return Array.Empty<byte>();
        }
