...
public partial class Histories : UserControl
{
    ...
    private ContextMenu CreateContextMenuForSingleCommit(ViewModels.Repository repo, Models.Commit commit)
    {
        ...
        if (!repo.IsBare)
        {
            ...
            if (commit.IsMerged && commit.Parents.Count > 0)
            {
                ...
                // Komorebi: 「Manually...」は選択コミット自身を on（exclusive base）として
                // InteractiveRebase(repo, commit) を開く実装だが、isHead の場合は on == HEAD となり
                // `on...HEAD` の範囲が常に空になってしまう（upstream 側にも同様の潜在問題がある）。
                // 実害のある空リストダイアログを避けるため、isHead では Manually を出さない。
                if (!isHead)
                {
        ...
        copyAuthorTime.Icon = App.CreateMenuIcon("Icons.DateTime");
        copyAuthorTime.Click += async (_, e) =>
        {
            await App.CopyTextAsync(Models.DateTimeFormat.Format(commit.AuthorTime));
            e.Handled = true;
        };

        var copyCommitterTime = new MenuItem();
        copyCommitterTime.Header = App.Text("CommitCM.CopyCommitterTime");
        copyCommitterTime.Icon = App.CreateMenuIcon("Icons.DateTime");
        copyCommitterTime.Click += async (_, e) =>
        {
            await App.CopyTextAsync(Models.DateTimeFormat.Format(commit.CommitterTime));
            e.Handled = true;
        };
    ...
    private static void FillOtherLocalBranchMenu(ContextMenu menu, ViewModels.Repository repo, Models.Branch branch, Models.Branch current, bool merged)
    {
        ...
        compare.Click += (_, e) =>
        {
            App.ShowWindow(new ViewModels.Compare(repo, current, branch));
            e.Handled = true;
        };
    ...
    private static void FillTagMenu(ContextMenu menu, ViewModels.Repository repo, Models.Tag tag, Models.Branch current, bool merged)
    {
        ...
        compare.Click += (_, e) =>
        {
            App.ShowWindow(new ViewModels.Compare(repo, current, tag));
            e.Handled = true;
        };
