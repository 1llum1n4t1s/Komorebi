...
public class ThemedTextDiffPresenter : TextEditor
{
    ...
    public class LineBackgroundRenderer : IBackgroundRenderer
    {
        ...
        {
            if (_presenter.Document is null || !textView.VisualLinesValid)
                return;
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (chunk is null || (!chunk.Combined && chunk.IsOldSide != IsOld))
            return;
...
public class SingleSideTextDiffPresenter : ThemedTextDiffPresenter
{
    ...
    private void OnTextViewScrollChanged(object sender, ScrollChangedEventArgs e)
    {
        if (_scrollViewer is null || DataContext is not ViewModels.TwoSideTextDiff diff)
            return;
