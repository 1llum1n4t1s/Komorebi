...
| **ロケール別フォント最適化** | 各言語環境に応じた OS 標準フォントを自動選択（日本語: Hiragino Sans / Yu Gothic UI 等） |
...
| **Git エラーヒント表示** | git コマンドが失敗した際、原因と対処方法を多言語ヒントで通知に表示。`dubious ownership`（`safe.directory` への追加）や鍵ファイルのパーミッション過多（`chmod 600`）はワンクリックで自動修正可能 |
...
| **アプリアイコン刷新** | 抽象的なクリスタル調のオリジナルアイコンに変更 |

### SSH 鍵管理・リモート設定

| 項目 | 内容 |
|------|------|
| **SSH 鍵管理 UI** | `~/.ssh/` をスキャンし、リモートごと・グローバルで SSH 鍵を選択可能。3 段フォールバック（リモート個別 → グローバル → ssh-agent）。SSH 鍵の一覧表示・生成にも対応 |
...
| 項目 | 内容 |
|------|------|
| **Avalonia 12 アップグレード** | Avalonia UI 12 系を採用 |
...
各プラットフォーム向けのインストーラを以下からダウンロードできます。Velopack による自動アップデートに対応しており、起動時に新しいバージョンがあれば通知されます。
...
| Linux | 新規環境は `$XDG_CONFIG_HOME/Komorebi`（未設定時は `~/.config/Komorebi`）。既存の `~/.komorebi` または `~/.config/Komorebi` がある場合は移行せず引き続き使用 |
...
| **GitFlow** | gitflow-next 対応 / Feature / Release / Hotfix の開始・完了 / Squash / Push / ブランチ保持オプション |
...
| **バイナリ差分** | 新旧サイズ表示・16進ビューア |
| **サブモジュール差分** | コミット範囲の表示 |
| **パッチ** | 保存 / 適用（空白処理モード: No Warn / Warn / Error / Error All） |

### 履歴・グラフ

* **独立ウィンドウ** — コミット詳細・リビジョン比較を別ウィンドウで表示可能
...
* **現在ブランチの情報を生成に反映** / **OpenAI 系の推論量設定**
...
リポジトリ / コミット / ブランチ / タグ / リモート / ファイルのスコープごとに独自のアクションを定義し、コンテキストメニューから実行できます。コントロール種別: テキストボックス（`${VALUE}` テンプレートによる文字列整形対応）/ パスセレクタ / チェックボックス / コンボボックス / ローカルブランチセレクタ / リモートブランチセレクタ。
...
LiveChartsCore による貢献統計グラフを表示します。全期間（月次）/ 今月（日次）/ 今週（日次）の 3 つのビューで、著者・ブランチによる絞り込みが可能です。
...
リポジトリごとにユーザー名・メール / デフォルトリモート / マージモード / GPG 署名 / HTTP プロキシ / 自動 Fetch（間隔設定・リモートごとの除外が可能）/ サブモジュール自動更新の確認 / リモートごとのプッシュ禁止 / コミットテンプレート / Issue トラッカー / AI サービス / カスタムアクション / Conventional Commit タイプ を個別に設定できます。
