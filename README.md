...
リポジトリごとにユーザー名・メール / デフォルトリモート / マージモード / GPG 署名 / HTTP プロキシ / 自動 Fetch（間隔設定可能）/ サブモジュール自動更新の確認 / リモートごとのプッシュ禁止 / コミットテンプレート / Issue トラッカー / AI サービス / カスタムアクション / Conventional Commit タイプ を個別に設定できます。
